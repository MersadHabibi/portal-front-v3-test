...
export default function Hero({ pageData }: { pageData: TPage | null }) {
  ...
  const [activeSubCategoryNumber, setActiveSubCategoryNumber] = useState<string | undefined>();
  const [isHovering, setIsHovering] = useState(false);

  const title = pageData?.keyValues.find(keyValue => keyValue.key.startsWith(KeyValues.HERO_TITLE))?.value || "";
  const image = pageData?.keyValues.find(keyValue => keyValue.key.startsWith(KeyValues.HERO_IMAGE))?.filesValue?.url;
  const logos = pageData?.keyValues.filter(keyValue => keyValue.key.startsWith(KeyValues.SUBCATEGORIES_LOGO));
  const details = pageData?.keyValues.filter(keyValue => keyValue.key.startsWith(KeyValues.SUBCATEGORIES_DETAIL));
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveSubCategoryNumber(prevNumber => {
        ...
        const currentIndex = logos.findIndex(logo => logo.key.endsWith(`-${prevNumber}`));
  ...
  useEffect(() => {
    ...
    const activeLogo = logos.find(logo => logo.key.endsWith(`-${activeSubCategoryNumber}`));
    if (!activeLogo) return;

    // Find the DOM element for the active logo
    const activeElement = containerRef.current?.querySelector(`[data-logo-key="${activeLogo.key}"]`) as HTMLElement;
    if (!activeElement || !containerRef.current || !parentRef.current) return;

    // Calculate scroll position similar to click behavior
    const MARGIN_OFFSET = (document.body.getBoundingClientRect().width < 648 ? 12 : 32) + 28;
    const elementLeftOffset = activeElement.getBoundingClientRect().left || 0;
    const parentElementLeftOffset = parentRef.current.getBoundingClientRect().left || 0;
    const leftOffset = elementLeftOffset - parentElementLeftOffset - MARGIN_OFFSET;

    const elementRightOffset = activeElement.getBoundingClientRect().right || 0;
    const parentElementRightOffset = parentRef.current.getBoundingClientRect().right || 0;
    const rightOffset = elementRightOffset - parentElementRightOffset + MARGIN_OFFSET;
  ...
  const onSubsetSelection = (e: MouseEvent<HTMLDivElement, globalThis.MouseEvent>, logo: any) => {
    const MARGIN_OFFSET = (document.body.getBoundingClientRect().width < 648 ? 12 : 32) + 28;
    ...
    const elementRightOffset = e.currentTarget.getBoundingClientRect().right || 0;
    const parentElementRightOffset = parentRef?.current?.getBoundingClientRect().right || 0;
  ...
  return (
    <div className="relative w-full flex flex-col justify-start items-center">
      ...
      {/* inner container */}
      <div className="z-[5] mt-10 flex flex-col justify-start items-center w-full max-w-[1090px] bg-transparent px-5 sm:px-3">
        ...
        <div ref={parentRef} className="flex justify-between items-center w-full z-[2]">
          {isScrolling && (
            <button onClick={() => scrollBy(145)} className="cursor-pointer relative top-[7px]">
              <ChevronRightCircle strokeWidth={1.2} className="size-[28px] text-primary" />
            </button>
          )}
          {/* logos container */}
          <div
            ...
            {!logos?.length && <p className="mx-auto">هنوز هیچ زیر مجموعه ای موجود نیست.</p>}
          ...
          {isScrolling && (
            <button onClick={() => scrollBy(-145)} className="cursor-pointer relative top-[7px]">
              <ChevronLeftCircle strokeWidth={1.2} className="size-[28px] text-primary" />
            </button>
          )}
        </div>
        ...
        {/* detailBox */}
        <div
          className="w-full bg-white [box-shadow:0_0_25px_rgba(0,0,0,0.1)] rounded-custom100 py-10 px-3 flex justify-center"
          onMouseEnter={() => setIsHovering(true)}
          onMouseLeave={() => setIsHovering(false)}>
          <InnerHTML
            ...
            details={details?.find(detail => detail.key.endsWith(`-${activeSubCategoryNumber}`))?.value || ""}
          />
        </div>
      </div>
    </div>
